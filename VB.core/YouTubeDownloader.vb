...
Public Class YouTubeVideoQuality
    ...
    Public Property Extention() As String
        ...
        End Set
    End Property
    <DebuggerBrowsable(DebuggerBrowsableState.Never)> Private m_Extention As String
    ''' <summary>
...
Public Class YouTubeDownloader
            ...
            For Each videoLink As Object In ExtractUrls(html)
                Try
                    ...
                    If Not getSize(q) Then
                        Continue For
                    End If
                    Dim s As String = "" 'WindowsApplication1.My.Resources.Regex_Youtube_length
                    s = """length_seconds"":""(.+?)"""
                    'Regex_Youtube_length	"length_seconds":"(.+?)",	
    ...
    Private Shared Function ExtractUrls(originalhtml As String) As List(Of String)
        ...
        Dim html As String = originalhtml.Substring(originalhtml.IndexOf("""url_encoded_fmt_stream_map"""))
        ' Uri.UnescapeDataString(Regex.Match(originalhtml, DataBlockStart, RegexOptions.Singleline).Groups(1).ToString())
        ...
        For Each match As Object In matchs
            ...
            Dim url1 As String = System.Net.WebUtility.UrlDecode(url)
            urls.Add(url1)
        Next
        ExtractUrls = urls
    End Function
